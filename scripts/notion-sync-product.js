...
async function assess(docsOutline, diff, docsIndex = []) {
  ...
  if (!plan) {
    console.log(
      chalk.yellow(`  No response from assessor ${chalk.dim(`(${elapsed}s)`)}`),
    );
    return null;
  }

  console.log(
    `  Meaningful: ${plan.meaningful ? chalk.green("yes") : chalk.yellow("no")} ${chalk.dim(`(${elapsed}s)`)}`,
  );
  ...
  if (plan.meaningful && plan.actions?.length) {
    ...
    for (const a of plan.actions) {
      const pageLabel =
        a.page_title || titleById.get(a.page_id) || a.page_id || "(new)";
      console.log(
        `    ${chalk.bold(a.type.padEnd(10))} ${chalk.magenta(pageLabel)}`,
      );
      console.log(
        chalk.dim(
          `       ${a.instructions.slice(0, 120)}${a.instructions.length > 120 ? "…" : ""}`,
        ),
      );
    }
  }
...
async function generate(actions, docsIndex, diff) {
  ...
  for (let i = 0; i < actions.length; i += CONCURRENCY) {
    ...
    console.log(
      chalk.dim(
        `  Batch ${Math.floor(i / CONCURRENCY) + 1}: ${batch.map((a) => a.page_title || a.type).join(", ")}`,
      ),
    );
    ...
    for (const r of batchResults) {
      ...
      const status = r.skipped
        ? chalk.yellow(`skipped (${r.skip_reason})`)
        : r.summary;
      const mdLen = r.markdown
        ? `${Math.round(r.markdown.length / 1024)}KB`
        : "0KB";
      console.log(
        `    ${icon} ${r.page_title}: ${status} ${chalk.dim(`[${mdLen}]`)}`,
      );
    }
...
async function main() {
  ...
  console.log(
    label(
      "Repository:",
      `${process.env.REPO_NAME} ${chalk.dim(`(${REPO_LABEL})`)}`,
    ),
  );
  ...
  console.log(
    label(
      "Changed:   ",
      `${changedFiles.length} files, ${Math.round(diff.length / 1024)}KB diff`,
    ),
  );
  ...
  console.log(
    chalk.dim(
      `  Docs outline: ${Math.round(docsOutline.length / 1024)}KB (headings only)`,
    ),
  );
  ...
  const validActions = plan.actions.filter((action) => {
    ...
    if (targetId && !validIds.has(targetId)) {
      console.warn(
        chalk.yellow(
          `  ⚠ Skipping ${action.type} on "${action.page_title}": page_id ${targetId} not found in Notion tree`,
        ),
      );
      return false;
    }
    ...
    const icon =
      entry.status === "ok"
        ...
        : entry.status === "error"
          ...
          : chalk.yellow("⚠");
    console.log(
      `  ${icon} ${chalk.bold((entry.type || "").padEnd(10))} "${entry.page}" ${chalk.dim(entry.id ? `[${entry.id}]` : "")}`,
    );
